...
def admin_dashboard(request):
    if 'user' not in request.session or request.session['user']['id'] != 1:
        ...
        print first_order.user_id.first_name
        return render(request, 'app2/admin_dashboard.html', content)
...
def make_purchase(request):
    ...
    messages.success(request, "Thank you for making your purchase. Your instructor will contact you shortly!")
    return redirect(index)

def deliver_order(request):
    order_to_change = Order.objects.get(id = request.POST['order_id'])
    order_to_change.status = True
